...
edge 机器                         公共互联网                         公司网络
+----------------+              +--------------------+            +-----------------+
| Codex CLI      |              | Nginx :443         |            | egress-agent    |
...
Nginx 是唯一公网 listener，通常使用 `443`；Node server-host 只能绑定 `127.0.0.1`，可使用任意
未占用的有效 TCP 端口。Edge 的本地应用 listener 仍固定在 `8000-9000`。网关 `443` 是 Agent 的受限
出站目标，不是应用 listener。
...
生产由 Nginx 终止公网 TLS，并由独立 `server-host` 进程在 `127.0.0.1` 组合 `ops` 受保护 bundle
与 `createTunnelServer`。唯一公开 URL 是 `wss://<publicHostname>/tunnel`；Nginx 只代理 `/health`
和 `/tunnel`，其他路径均为 `404`。Nginx 覆盖 `X-Forwarded-For` 为 `$remote_addr`，Node 仅在后端
socket 为 `127.0.0.1`、header 恰好一个且为合法 IP 时以它实施连接限流。证书通过预置流程或 DNS-01
续期并在 `SIGHUP` 后安全 reload；host 配置、Origin、TLS 下限、代理地址来源或资源限制变化必须重启。
