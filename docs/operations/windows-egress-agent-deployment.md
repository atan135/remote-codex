...
- 到唯一 Public Server `wss://<hostname>:<8000-9000>/tunnel` 的 WSS；标准端口是 `8443`。
...
release、命令参数、环境变量、任务描述或日志。配置中的 Public Server URL 必须显式使用
`8000-9000` 内端口和 `/tunnel`；模型网关必须是非 IP 的精确 hostname 和 `443`。
...
agent 用户可出站访问 Public Server 固定端口（例如 `8443`）和模型网关 `443`，没有 Remote Codex
