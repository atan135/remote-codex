...
`127.0.0.1:<8000-9000>` 的 HTTP CONNECT。样例使用直连 Public Server `8443`、本地 listener
`8787`。它不提供 SOCKS、普通 HTTP 转发、文件、命令、调试或管理接口，不修改 Windows 系统
代理、防火墙或全局环境变量，也不直连模型网关。WSS 客户端从唯一
`wss://<hostname>:8443/tunnel` 固定派生并发送对应 HTTPS Origin；Origin
...
已安装 PM2 的当前普通用户可选择 PM2 代替上述登录任务。PM2 只管理固定的
`remote-codex-edge-client` 进程，不提供远程控制、PM2 Plus 或额外网络 endpoint。先停止并卸载
现有 `RemoteCodex-EdgeClient` 任务；两种方式同时运行会争用同一个 loopback listener：

```powershell
.\deployment\windows\edge-client\Stop-EdgeClientTask.ps1
.\deployment\windows\edge-client\Uninstall-EdgeClientTask.ps1

.\deployment\windows\pm2\Start-EdgeClientPm2.ps1 `
  -ReleaseRoot C:\RemoteCodex\current `
  -ConfigRoot "$env:LOCALAPPDATA\RemoteCodex\edge-client"
```

启动脚本会先执行 `deployment validate`，然后等价于对固定 host 入口执行 `pm2 start`。Node 由 PM2
后台托管，不会保留 Node 控制台窗口。日常操作使用：

```powershell
.\deployment\windows\pm2\Restart-EdgeClientPm2.ps1
.\deployment\windows\pm2\Stop-EdgeClientPm2.ps1
pm2 status remote-codex-edge-client
pm2 logs remote-codex-edge-client
```

PM2 模式不使用 `Test-EdgeClientStatus.ps1`，因为该脚本明确验证 Windows 任务状态。通过 `pm2 status`
确认进程在线后，继续按下文的 loopback 和真实网络验收步骤检查；不得使用 PM2 启动其他服务、修改
`HTTPS_PROXY` 的系统级值或绕过启动前的配置校验。

## Codex 会话接入

Edge 进程在线后，只对需要使用隧道的当前终端进程设置 `HTTPS_PROXY`。端口必须与受保护配置完全一致：
