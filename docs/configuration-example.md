...
- `serverUrl` 使用固定 `/tunnel` 且不含凭据、查询和 fragment 的 `wss:` URL。Nginx 默认
  `443` 时写作 `wss://<hostname>/tunnel`；非标准端口可以显式写出任意有效 TCP 端口。非浏览器
  agent/edge 客户端发送对应的 HTTPS Origin；Origin 不是身份凭据。
