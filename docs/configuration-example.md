...
在 runtime 在线后，将它传给 `LoopbackConnectProxy` 作为 `streamGateway`，并以
`listenPort` 创建本地 CONNECT 监听器。
...
- `serverUrl` 只使用显式 `8000-9000` 端口、固定 `/tunnel` 且不含凭据、查询和 fragment 的
  `wss:` URL。非浏览器 agent/edge 客户端发送对应的 HTTPS Origin；Origin 不是身份凭据。
