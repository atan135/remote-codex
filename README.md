...
Agent 没有入站面；Edge 只有 `127.0.0.1:<8000-9000>` CONNECT listener。Public Server 由 Nginx
在 `443` 终止公网 TLS，并反代到仅 loopback 的 Node TLS/WSS listener；两端 WSS 使用
`wss://<hostname>/tunnel`。模型网关仍只允许精确 hostname 的出站 `443`。
...
Public Server 的唯一公开 WSS URL 为 `wss://<publicHostname>/tunnel`，同一入口只额外提供无凭据
`GET /health`。Nginx 只监听 `443`，只转发这两个路径到 Node 的 `127.0.0.1:<任意有效端口>`；
不提供 `80`、静态文件、管理或 metrics endpoint。Node 后端默认样例端口为 `8443`，但生产可使用任意
未占用端口。Linux `systemd` 单元、Nginx TLS/反代配置、证书续期与安全 reload、手工检查和回滚流程见
